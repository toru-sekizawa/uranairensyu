...
@app.route('/', methods=['GET', 'POST'])
def results():
    years = requests.form.get('years')
    month = requests.form.get('month')
    days = requests.form.get('days')

    df = pd.read_csv(f'data/{years}.csv')
    days = int(days)
    month = int(month)
    meisu = df.iloc[days - 1, month]
    meisu = meisu.replace('-', '')
    ...
    if age >= 60:
        if meisu[0:2] < '11':
            ...
            print(f'あなたは{even_odd}のカメレオンです。')
